...
import { Row, Col, Breadcrumb, Badge, Dropdown, Button, List, Avatar, Input, Drawer, Typography, Switch, Menu } from 'antd';

import { PlusOutlined, BellOutlined, StarOutlined, TwitterOutlined, FacebookFilled, DownOutlined, FileAddOutlined } from '@ant-design/icons';
...
function Header({ placement, name, subName, onPress, handleSidenavColor, handleSidenavType, handleFixedNavbar, currentUser, breakcrumbUrl, invites }) {
	...
	const menu = (
		<List
			...
			dataSource={invites.map((invite, index) => {
				return {
					title: "Bạn được thêm vào lớp: " + invite.classroom.name,
					description: <>{clockicon} <Moment>{invite.updatedAt}</Moment> </>,
					avatar: <Avatar shape='square'>{wifi}</Avatar>,
				}
			})}
			renderItem={(item) => (
