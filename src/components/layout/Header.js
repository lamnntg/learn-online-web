...
function Header({
  ...
  const [chooseInvite, setChooseInvite] = useState(null);
    ...
    <List
      ...
      dataSource={invites.map((invite, index) => {
        return {
          ...
          description: (
            <>
              {clockicon} <Moment>{invite.updatedAt}</Moment>{" "}
            </>
          ),
          avatar: <Avatar shape="square">{wifi}</Avatar>,
          invite: invite,
  ...
  const showModalSubmit = (item) => {
    setChooseInvite(item.invite);
  ...
  return (
    <>
      ...
      >
        <p>
          Lời mời tham gia lớp : {chooseInvite?.classroom?.name} - Phòng:{" "}
          {chooseInvite?.classroom?.room}. Môn :{" "}
          {chooseInvite?.classroom?.subject}
        </p>
      </Modal>
    </>
  );
