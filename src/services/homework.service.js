...
export const createExamPDF = async data => {
  const req = await axios.post(`${API_ROOT}/homework/create/by-pdf`, data, { headers: authHeader() });

  return req.data;
};

export const uploadPDF = async data => {
  const req = await axios.post(`https://v2.convertapi.com/convert/pdf/to/png?Secret=${CONVERT_API_SECRET_KEY}&StoreFile=true`, data);
  return req.data;
};
